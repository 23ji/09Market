...
classDiagram
    ...
    %% ── Domain Layer : UseCase (Implement - DomainImpl) ──
    class CheckAuthOnLaunchUseCaseImpl {
        -authRepository · userRepository · userStore
    }
    class SignInWithIdTokenUseCaseImpl {
        -authRepository · userRepository · userStore
    }
    class SignOutUseCaseImpl {
        -authRepository · userStore
    }
    ...
    class AuthReactor {
        <<FactoryModule>>
        +Dependency : CheckAuthOnLaunchUseCase
        Action : checkAuth
    ...
    %% ── Relationships ──
    CheckAuthOnLaunchUseCaseImpl ..|> CheckAuthOnLaunchUseCase
    CheckAuthOnLaunchUseCaseImpl --> AuthRepository
    CheckAuthOnLaunchUseCaseImpl --> UserRepository
    CheckAuthOnLaunchUseCaseImpl --> UserStore

    SignInWithIdTokenUseCaseImpl ..|> SignInWithIdTokenUseCase
    SignInWithIdTokenUseCaseImpl --> AuthRepository
    SignInWithIdTokenUseCaseImpl --> UserRepository
    SignInWithIdTokenUseCaseImpl --> UserStore

    SignOutUseCaseImpl ..|> SignOutUseCase
    SignOutUseCaseImpl --> AuthRepository
    SignOutUseCaseImpl --> UserStore
    ...
    AuthCoordinator --> AuthViewController
...
graph TB
    ...
    Data[Data]
    DomainI[Domain<br/>Interface]
    DomainImpl[Domain<br/>Implement]
    Core[Core]
    Util[Util]
    DS[DesignSystem]
    SharedRx[Shared_ReactiveX]
    SharedDI[Shared_DI]
    ...
    App --> Data
    App --> DomainI & DomainImpl
    App --> Core & Util & DS
    App --> SharedDI

    AuthImpl --> AuthI & DomainI & Core & Util & SharedRx & SharedDI
    LoginImpl --> LoginI & DomainI & Core & Util & DS & SharedRx & SharedDI
    HomeImpl --> HomeI & DomainI & Core & SharedDI
    ProfileImpl --> ProfileI & DomainI & Core & DS & SharedRx & SharedDI & Util
    ...
    Data --> DomainI & Core & SharedDI
    DomainI --> Core & Util
    DomainImpl --> Core & SharedDI
    Core --> Util & SharedRx
    DS --> Util
```
...
| 파일 | 클래스/타입 | 역할 |
|------|------------|------|
| `Util/Sources/Parsers/JWTDecoder.swift` | `JWTDecoder` | JWT payload의 `exp` claim으로 토큰 만료 판별 |
| `Util/Sources/Constants/Strings.swift` | `Strings` | 앱 전역 문자열 상수 |

#### DesignSystem 레이어

| 파일 | 클래스/타입 | 역할 |
|------|------------|------|
| `DesignSystem/Sources/Components/LoadingIndicator.swift` | `LoadingIndicator` | 로딩 인디케이터 표시/숨김 |
| `DesignSystem/Sources/Components/ConfirmDialog.swift` | `ConfirmDialog` | 확인/취소 다이얼로그 |

#### Domain 레이어

> Domain은 Interface(`Domain`)와 Implement(`DomainImpl`) 타겟으로 분리되어 있다.

| 파일 | 클래스/타입 | 역할 |
|------|------------|------|
| `Domain/Sources/Interface/Entity/Auth/AuthState.swift` | `AuthState` (enum) | 앱 인증 상태: `anonymous` / `authenticated(User)` / `unauthenticated` |
| `Domain/Sources/Interface/Entity/Auth/AuthToken.swift` | `AuthToken` | JWT 토큰 보관. `isExpired` 프로퍼티로 만료 판별 |
| `Domain/Sources/Interface/Entity/Auth/TokenStatus.swift` | `TokenStatus` (enum) | 토큰 평가 결과: `noToken` / `valid` / `expired` |
| `Domain/Sources/Interface/Repository/Auth/AuthRepository.swift` | `AuthRepository` (protocol) | 인증 관련 Repository 인터페이스 |
| `Domain/Sources/Interface/Usecase/Auth/CheckAuthOnLaunchUseCase.swift` | `CheckAuthOnLaunchUseCase` (protocol) | 앱 실행 시 인증 상태 판별 인터페이스 |
| `Domain/Sources/Interface/Usecase/Auth/SignInWithIdTokenUseCase.swift` | `SignInWithIdTokenUseCase` (protocol) | 소셜 로그인 인터페이스 |
| `Domain/Sources/Interface/Usecase/Auth/SignOutUseCase.swift` | `SignOutUseCase` (protocol) | 로그아웃 인터페이스 |
| `Domain/Sources/Interface/Usecase/Auth/DeleteAccountUseCase.swift` | `DeleteAccountUseCase` (protocol) | 계정 삭제 인터페이스 |
| `Domain/Sources/Implement/Usecase/Auth/CheckAuthOnLaunchUseCaseImpl.swift` | `CheckAuthOnLaunchUseCaseImpl` | 앱 실행 시 인증 상태 판별 (핵심 로직) |
| `Domain/Sources/Implement/Usecase/Auth/SignInWithIdTokenUseCaseImpl.swift` | `SignInWithIdTokenUseCaseImpl` | 소셜 로그인: idToken → Supabase 인증 → 유저 조회 |
| `Domain/Sources/Implement/Usecase/Auth/SignOutUseCaseImpl.swift` | `SignOutUseCaseImpl` | 로그아웃: SDK + Supabase 로그아웃 → 익명 전환 |
| `Domain/Sources/Implement/Assembly/DomainAssembly.swift` | `DomainAssembly` | UseCase 구현체 DI 등록 |
...
> Reactor는 `FactoryModule` (Pure 라이브러리)을 채택하여 `Dependency` 구조체로 의존성을 선언한다.
> Assembly에서 Factory 체인으로 등록하고 (`.graph` scope), CoordinatorImpl이 조립된 인스턴스를 수신한다.

| 파일 | 클래스/타입 | 역할 |
|------|------------|------|
| `Features/Authenticate/.../AuthCoordinatorImpl.swift` | `AuthCoordinatorImpl` | Splash 표시 → AuthReactor 구독 → delegate 호출 |
| `Features/Authenticate/.../AuthReactor.swift` | `AuthReactor` (ReactorKit, FactoryModule) | `.checkAuth` 액션 → UseCase 실행 → 상태 업데이트 |
| `Features/Authenticate/.../AuthViewController.swift` | `AuthViewController` | Splash UI (인증 확인 중 표시) |
| `Features/Authenticate/.../AuthAssembly.swift` | `AuthAssembly` | AuthReactor.Factory, AuthCoordinator DI 등록 |
